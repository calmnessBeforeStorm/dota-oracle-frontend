...
- Коммиты подписываются почтой **`ersaimadilet@yandex.kz`** (локальный `user.email` уже
  прописан в репозитории, не перезаписывать глобальным). Это адрес для GitHub, где живёт
  `origin`; до 14.09.2026 здесь стоял `ersaim.adilet@yandex.kz`, и вся история старше этой
  даты подписана им.
